...
const Navbar: React.FC<NavbarProps> = ({ 
  ...
  const [isQuartzOpen, setIsQuartzOpen] = useState(false);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20 items-center">
          ...
          <div className="hidden lg:flex items-center justify-center flex-grow space-x-2 xl:space-x-4 mx-4">
            ...
            <div 
              className="relative group"
              onMouseEnter={() => setIsQuartzOpen(true)}
              onMouseLeave={() => setIsQuartzOpen(false)}
            >
              <button
              ...
              >
                Quartz <ChevronRight size={14} className={`ml-1 transition-transform ${isQuartzOpen ? 'rotate-90' : ''}`} />
              </button>
              
              {isQuartzOpen && (
                <div className="absolute top-full left-0 w-64 bg-white shadow-xl border border-gray-100 py-2 rounded-sm animate-in fade-in slide-in-from-top-2 duration-200">
                  <button 
                    onClick={() => { navigateTo('Inverness', 'printing'); setIsQuartzOpen(false); }}
                    className="w-full text-left px-4 py-3 text-xs font-bold uppercase tracking-wider hover:bg-gray-50 transition-colors text-gray-600 hover:text-royal-blue"
                  >
                    Printing Quartz
                  </button>
                  <button 
                    onClick={() => { navigateTo('Inverness', 'popular'); setIsQuartzOpen(false); }}
                    className="w-full text-left px-4 py-3 text-xs font-bold uppercase tracking-wider hover:bg-gray-50 transition-colors text-gray-600 hover:text-royal-blue"
                  >
                    Popular
                  </button>
                  <button 
                    onClick={() => { navigateTo('Inverness', 'luxe'); setIsQuartzOpen(false); }}
                    className="w-full text-left px-4 py-3 text-xs font-bold uppercase tracking-wider hover:bg-gray-50 transition-colors text-gray-600 hover:text-royal-blue"
                  >
                    Luxe Series
                  </button>
                  <button 
                    onClick={() => { navigateTo('Inverness', 'ultralux'); setIsQuartzOpen(false); }}
                    className="w-full text-left px-4 py-3 text-xs font-bold uppercase tracking-wider hover:bg-gray-50 transition-colors text-gray-600 hover:text-royal-blue"
                  >
                    Ultra Lux
                  </button>
                  <button 
                    onClick={() => { navigateTo('Inverness', 'translucent'); setIsQuartzOpen(false); }}
                    className="w-full text-left px-4 py-3 text-xs font-bold uppercase tracking-wider hover:bg-gray-50 transition-colors text-gray-600 hover:text-royal-blue"
                  >
                    Translucent Quartz
                  </button>
      ...
      {isOpen && (
        <div className="lg:hidden bg-white border-b border-gray-100 py-6 px-4 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 border-b border-gray-50 pb-1">Quartz Collections</p>
              <div className="grid grid-cols-2 gap-2">
                <button 
                  onClick={() => { navigateTo('Inverness', 'printing'); setIsOpen(false); }} 
                  className="text-left px-4 py-3 text-xs font-bold rounded-sm border border-gray-100 text-gray-600 hover:text-royal-blue"
                >
                  Printing Quartz
                </button>
                <button 
                  onClick={() => { navigateTo('Inverness', 'popular'); setIsOpen(false); }} 
                  className="text-left px-4 py-3 text-xs font-bold rounded-sm border border-gray-100 text-gray-600 hover:text-royal-blue"
                >
                  Popular
                </button>
                <button 
                  onClick={() => { navigateTo('Inverness', 'luxe'); setIsOpen(false); }} 
                  className="text-left px-4 py-3 text-xs font-bold rounded-sm border border-gray-100 text-gray-600 hover:text-royal-blue"
                >
                  Luxe Series
                </button>
                <button 
                  onClick={() => { navigateTo('Inverness', 'ultralux'); setIsOpen(false); }} 
                  className="text-left px-4 py-3 text-xs font-bold rounded-sm border border-gray-100 text-gray-600 hover:text-royal-blue"
                >
                  Ultra Lux
                </button>
                <button 
                  onClick={() => { navigateTo('Inverness', 'translucent'); setIsOpen(false); }} 
                  className="text-left px-4 py-3 text-xs font-bold rounded-sm border border-gray-100 text-gray-600 hover:text-royal-blue col-span-2"
                >
                  Translucent Quartz
                </button>
              </div>
