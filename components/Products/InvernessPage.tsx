...
const InvernessPage: React.FC<InvernessPageProps> = ({ navigateTo, products }) => {
  ...
  // Effect to handle URL hash for Quartz category page views
  useEffect(() => {
    const handleHashChange = () => {
      const hash = window.location.hash.replace('#', '').toLowerCase();
      if (hash) {
        if (hash === 'printing') setSelectedCategory('Printing');
        else if (hash === 'popular') setSelectedCategory('Popular');
        else if (hash === 'luxe' || hash === 'ultralux') setSelectedCategory('UltraLux');
        else if (hash === 'translucent') setSelectedCategory('Translucent');
      } else {
  ...
  if (selectedCategory) {
    const items = QUARTZ_DATA[selectedCategory as keyof typeof QUARTZ_DATA] || [];
    const displayName = selectedCategory === 'UltraLux' ? 'Luxe & Ultra Lux' : selectedCategory;
      ...
      <div className="min-h-screen bg-white py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center space-x-2 text-xs uppercase tracking-widest text-gray-400 mb-8">
            ...
            <span>/</span>
            <span className="text-gray-900 font-bold">{displayName} Quartz</span>
          </div>
          
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 border-b border-gray-100 pb-8">
            <div className="space-y-4">
              <h2 className="text-4xl font-bold text-brand-black uppercase tracking-tighter">
                {displayName} Quartz Collection
              </h2>
              <p className="text-gray-500 max-w-xl">
                Browse our premium selection of {displayName} Quartz surfaces, curated for luxury spaces and high-end design layouts.
              </p>
            </div>
            <button 
              onClick={() => {
              ...
              className="border border-brand-black text-brand-black hover:bg-brand-black hover:text-white px-6 py-3 text-xs font-bold uppercase tracking-wider transition-all duration-300"
            >
              Back to Catalog
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            {items.map((item, idx) => {
              const cleanName = item.name.replace(/\bvida\b/gi, '').replace(/\bsh\b/gi, 'Signature').replace(/%26/g, '&').replace(/\s+/g, ' ').trim();
              return (
                <motion.div
                  key={item.id}
                ...
                >
                  <div className="relative aspect-[4/3] overflow-hidden bg-gray-50 border border-gray-100 group-hover:shadow-2xl transition-all duration-500 mb-4">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 bg-white/90 backdrop-blur-sm">
                      <p className="text-[9px] font-bold uppercase tracking-widest text-royal-blue text-center">View Details</p>
                    </div>
                  </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-12 md:py-24">
        ...
        <div className="mt-32 space-y-12">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-4">
              ...
              <p className="text-gray-500 max-w-xl">Explore our exclusive range of Quartz surfaces, designed for timeless elegance, premium finishes, and superior tactile quality.</p>
            </div>
        ...
        <div className="mt-24 space-y-24">
          {Object.entries(QUARTZ_DATA).map(([category, items]) => {
            ...
            return (
              <div key={category} id={`quartz-${categoryId}`} style={{ scrollMarginTop: '6.5rem' }} className="space-y-8">
                <div className="flex items-center justify-between">
                  <h3 className="text-3xl font-bold text-brand-black uppercase tracking-tighter">
                    {category === 'UltraLux' ? 'Luxe & Ultra Lux' : category} Quartz
                  </h3>
                  <button 
                    onClick={() => {
                      const hashId = category.toLowerCase().replace(/\s+/g, '');
                      window.location.hash = hashId;
                    }}
                    className="flex items-center space-x-2 text-royal-blue font-bold text-xs uppercase tracking-widest hover:underline group/opt cursor-pointer"
                  >
                    <span>{items.length} Options</span>
                    <div className="w-8 h-0.5 bg-royal-blue group-hover/opt:w-12 transition-all duration-300" />
                  </button>
                </div>
                  ...
                  {items.map((item) => (
                    ...
                    >
                      <div className="relative aspect-[4/3] overflow-hidden bg-gray-50 border border-gray-100 group-hover:shadow-2xl transition-all duration-500 mb-4">
                        <img 
                          src={item.image} 
                          alt={item.name.replace(/\bvida\b/gi, '').replace(/\bsh\b/gi, 'Signature').replace(/%26/g, '&').replace(/\s+/g, ' ').trim()} 
                          className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" 
                        />
                        <div className="absolute inset-0 bg-black/10 group-hover:bg-transparent transition-colors" />
                      </div>
                      <h4 className="text-sm font-bold text-gray-900 uppercase tracking-tight group-hover:text-royal-blue transition-colors truncate">
                        {item.name.replace(/\bvida\b/gi, '').replace(/\bsh\b/gi, 'Signature').replace(/%26/g, '&').replace(/\s+/g, ' ').trim()}
                      </h4>
                    </motion.div>
                  ))}
                </div>
